#!/usr/bin/env tsx
/**
 * Drift detection for Kenya Law MCP.
 *
 * Checks if upstream Kenya Law content has changed since last ingestion.
...
async function main(): Promise<void> {
  console.log('Kenya Law MCP — Drift Detection');
  console.log('================================\n');
  ...
  for (const hash of fixture.provisions) {
    ...
    try {
      const response = await fetch(hash.upstream_url, {
        headers: { 'User-Agent': 'Kenya-Law-MCP/1.0 drift-detect' },
      });
