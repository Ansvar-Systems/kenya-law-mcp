/**
 * search_eu_implementations — Search EU directives/regulations that have Kenyan implementing legislation.
 */
...
export interface SearchEUImplementationsInput {
  ...
  year_from?: number;
  year_to?: number;
  has_kenyan_implementation?: boolean;
  limit?: number;
}
...
export async function searchEUImplementations(
  ...
  let sql = `
    SELECT
      ...
      ed.title,
      ed.short_name,
      COUNT(DISTINCT er.document_id) as kenyan_statute_count
  ...
  if (input.has_kenyan_implementation) {
    sql += ' HAVING kenyan_statute_count > 0';
  }
