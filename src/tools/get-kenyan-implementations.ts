/**
 * get_kenyan_implementations — Find Kenyan statutes implementing a specific EU directive/regulation
...
export interface GetKenyanImplementationsInput {
  eu_document_id: string;
  primary_only?: boolean;
  in_force_only?: boolean;
}

export interface KenyanImplementationResult {
  document_id: string;
...
export async function getKenyanImplementations(
  db: InstanceType<typeof Database>,
  input: GetKenyanImplementationsInput,
): Promise<ToolResponse<KenyanImplementationResult[]>> {
  try {
  ...
  const rows = db.prepare(sql).all(...params) as KenyanImplementationResult[];
